...
// No overload matches this call. Overload 1 of 2, '(value: string): string', gave the following error. Argument of type 'boolean' is not assignable to parameter of type 'string'.
// Overload 2 of 2, '(value: number): number', gave the following error. Argument of type 'boolean' is not assignable to parameter of type 'number'.
//processInput(true);

// hatte vorher diesen Fehler bekommen: This overload signature is not compatible with its implementation signature.
// Grund: in der Implementation-Signature waren beide Argumente zwingend verlangt also firstName und lastName, wenn man aber eine Overload Funktion hat die nicht alle
// Argumente aus der Implementation-Signature hat, muss diese als optional gekennzeichnet werden mit ?, deswegen: function getUserName(firstName: string, lastName?: string): string {}
function getUserName(firstName: string): string;
