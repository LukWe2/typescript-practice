...
// hier holen wir uns die Typen der Properties im User Type, da der Aufruf "Typname["Property-Name"]" in TypeScript den Typ der Property holt
// In JavaScript würde man wenn "user" ein Objekt wäre mit user["name"] auf den Wert von Property name zugreifen
// In TypeScript arbeitet man mit diesem Zugriff im TypeScript-Typsystem und liefert einen Typen, nicht die Großschreibung macht es aus sondern einfach dass es ein Typ in TypeScript ist
// aber auch in TypeScript funktioniert der Wertezugriff eines Objektes so, wenn man es auf ein Objekt aufruft diesen Aufruf
type NameType = User["name"]; // string -> und zwar nur string, kein Objekt mit type NameType = { x: string } oder so
type IdType = User["id"]; // number
type ActiveType = User["active"]; // boolean

// jetzt benutzen wir diesen Aufruf und kombinieren es mit keyof, heißt der Aufruf wäre hier: type UserValues = User["id" | "name" | "email" | "active"] -> Ergebnis dieses Aufrufs: type UserValues = number | string (| string) | boolean
// also holen uns alle Typen der Properties aus dem User Type
type UserValues = User[keyof User]; // "id" | "name" | "email" | "active"

let value: UserValues; // "id" | "name" | "email" | "active"
...
// bedeutet: Welchen Typ haben die Elemente dieses Arrays? Weil wir mit dem Wert in den eckigen Klammern bei Arrays auf Elemente zugreifen wir bei array[0] auf das erste Element, mit array[1] auf das zweite usw.
// number bedeutet hier nicht speziell Index 0 oder 1, sondern sinngemäß: Welchen Typ bekommt man, wenn man dieses Array mit irgendeinem gültigen numerischen Index ausliest? Da jedes Element ein User ist, ist UserArray[number] gleich Typ User
// heißt der Index number ist hier keine Zahl oder Zugriff sondern ein Platzhalter der sagt, welcher Typ geht hier überhaupt sinnesgemäß dem Typen die die Elemente des Arrays haben, da wir vorher  type UserArray = User[]; gemacht haben ist es User
